...
VALUES
  ('Whiskey Sour', 'Classic cocktail with bourbon', 12.99,
   (SELECT id FROM categories WHERE name='Cocktails'),
...
VALUES
  ('Amber Lager', 'Crisp lager with caramel notes', 6.99,
   (SELECT id FROM categories WHERE name='Lagers & Ales'),
...
VALUES
  ...
  ('Bruschetta', 'Tomato basil on toasted baguette', 6.99,
   (SELECT id FROM categories WHERE name='Small Plates'),
...
VALUES
  ('House Lager', 'Crisp pub lager', 5.99,
   (SELECT id FROM categories WHERE name='Drafts'),
  ...
  ('Wings', 'Buffalo wings, 8pc', 9.99,
   (SELECT id FROM categories WHERE name='Pub Fare'),
