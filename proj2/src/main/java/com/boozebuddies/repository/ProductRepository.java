...
public interface ProductRepository extends JpaRepository<Product, Long> {
  ...
  @Transactional
  @Query(
      "UPDATE Product p SET p.stockQuantity = p.stockQuantity - :qty WHERE p.id = :productId AND (p.stockQuantity IS NULL OR p.stockQuantity >= :qty)")
  int tryReserveStock(@Param("productId") Long productId, @Param("qty") int qty);

  @Modifying
  @Transactional
  @Query(
      "UPDATE Product p SET p.stockQuantity = p.stockQuantity + :qty WHERE p.id = :productId AND p.stockQuantity IS NOT NULL")
