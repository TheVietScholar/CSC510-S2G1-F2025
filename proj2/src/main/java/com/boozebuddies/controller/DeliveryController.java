...
public class DeliveryController {
    ...
    public ResponseEntity<ApiResponse<DeliveryDTO>> updateDeliveryStatus(
        ...
        try {
            ...
            DeliveryDTO deliveryDTO = convertToDTO(delivery);
            return ResponseEntity.ok(ApiResponse.success(deliveryDTO, "Delivery status updated successfully"));
        } catch (Exception e) {
            return ResponseEntity.badRequest()
    ...
    public ResponseEntity<ApiResponse<List<DeliveryDTO>>> getDeliveriesByDriver(@PathVariable Long driverId) {
        try {
            ...
            return ResponseEntity.ok(ApiResponse.success(deliveryDTOs, "Deliveries retrieved successfully"));
        } catch (Exception e) {
            return ResponseEntity.badRequest()
    ...
    public ResponseEntity<ApiResponse<List<DeliveryDTO>>> getActiveDeliveries() {
        try {
            ...
            List<DeliveryDTO> deliveryDTOs = activeDeliveries.stream()
                    .map(this::convertToDTO)
                    .collect(Collectors.toList());
            return ResponseEntity.ok(ApiResponse.success(deliveryDTOs, "Active deliveries retrieved successfully"));
        } catch (Exception e) {
            return ResponseEntity.badRequest()
