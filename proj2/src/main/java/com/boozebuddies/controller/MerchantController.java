...
public class MerchantController {
    ...
    public ResponseEntity<ApiResponse<MerchantDTO>> registerMerchant(@RequestBody MerchantDTO merchantDTO) {
        try {
            ...
            return ResponseEntity.ok(ApiResponse.success(responseDTO, "Merchant registered successfully"));
        } catch (Exception e) {
    ...
    public ResponseEntity<ApiResponse<MerchantDTO>> verifyMerchant(
        ...
        try {
            ...
            String message = verified ? "Merchant verified successfully" : "Merchant verification revoked";
    ...
    public ResponseEntity<ApiResponse<MerchantDTO>> updateInventory(
            @PathVariable Long merchantId,
            @RequestParam String inventoryDetails) {
        try {
    ...
    public ResponseEntity<ApiResponse<List<MerchantDTO>>> getAllMerchants() {
        try {
            ...
            List<MerchantDTO> merchantDTOs = allMerchants.stream()
                    .map(this::convertToDTO)
                    .collect(Collectors.toList());
            return ResponseEntity.ok(ApiResponse.success(merchantDTOs, "All merchants retrieved successfully"));
        } catch (Exception e) {
            return ResponseEntity.badRequest()
    ...
    public ResponseEntity<ApiResponse<List<OrderDTO>>> getOrdersByMerchant(@PathVariable Long merchantId) {
        try {
            List<Order> orders = merchantService.getOrdersByMerchant(merchantId);
            List<OrderDTO> orderDTOs = orders.stream()
                    .map(this::convertOrderToDTO)
                    .collect(Collectors.toList());
            return ResponseEntity.ok(ApiResponse.success(orderDTOs, "Merchant orders retrieved successfully"));
        } catch (Exception e) {
            return ResponseEntity.badRequest()
